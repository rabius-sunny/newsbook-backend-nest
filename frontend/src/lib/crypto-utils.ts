...
/**
 * Verifies and decrypts signed data
...
/**
 * Async version of signKey using Web Crypto API for stronger encryption
 * @param data - The data to encrypt
 * @param secret - Optional secret key (defaults to DEFAULT_SECRET)
 * @returns Promise that resolves to encrypted base64 string
 */
export async function signKeyAsync(
  data: string | number | boolean | object | Array<any>,
...
/**
 * Async version of verifyKey using Web Crypto API for stronger decryption
 * @param signedData - The encrypted data to decrypt
 * @param secret - Optional secret key (defaults to DEFAULT_SECRET)
 * @returns Promise that resolves to original data or null if verification fails
 */
export async function verifyKeyAsync(
  signedData: string,
  secret: string = DEFAULT_SECRET,
): Promise<string | null> {
  try {
