...
import { baseCategorySchema, CategorySchema } from '@/lib/validations/schemas/category.schema'
...
import type { TCategoryTree } from '../../../../api-types'
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      ...
      if (initialData?.id) {
        ...
        response = await requests.put(`/categories/${initialData.id}`, data)
      } else {
        // Create new category
        response = await requests.post('/categories', data)
      }
      ...
      if (response?.success) {
        ...
        showError(response?.message || `Failed to ${initialData ? 'updated' : 'created'} category`)
      }
    } catch (error) {
      showError(error)
    } finally {
      ...
      <form onSubmit={onSubmit} className='space-y-6'>
                    ...
                    {selectedParentData && (
                      <div className='bg-muted/50 p-2 border rounded-md text-muted-foreground text-sm'>
                        ...
                        {selectedParentData.parentPath.length > 0 && (
                          <span>{selectedParentData.parentPath.join(' → ')} → </span>
                        )}
        ...
        <div className='flex justify-end items-center gap-4 pt-6 border-t'>
          ...
          <Button type='submit' disabled={totalIsLoading}>
            <Save className='mr-2 w-4 h-4' />
            {totalIsLoading ? 'Saving...' : initialData ? 'Update Category' : 'Create Category'}
          </Button>
        </div>
      </form>
    </div>
