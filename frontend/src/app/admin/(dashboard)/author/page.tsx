...
import type { Author, TAuthorList } from '@/types/author.types';
...
export function AuthorsPageContent() {
  ...
  } = useAsync<TAuthorList>('/admin/users/authors');
  ...
  const actions: ActionItem<Author>[] = useMemo(
    () => [
      {
        label: 'Edit',
        onClick: (item) => {
          // Root admin cannot be edited by anyone
          if (isRootAdmin(item.id)) {
            toast.error('Root admin cannot be modified');
            return;
          }
          // Only admins can edit other users
          if (!isAdmin && adminInfo?.id !== item.id) {
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground">
              Manage authors and their access levels
            </p>
          </div>
          {isAdmin && (
