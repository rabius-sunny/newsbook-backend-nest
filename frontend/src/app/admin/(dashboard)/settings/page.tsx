...
const ImageCard = ({ value, name }: { value: string; name: string }) => (
  <div className="group flex flex-col gap-y-2 bg-white p-2 rounded-md w-28 overflow-hidden">
    <span className="text-muted-foreground text-xs text-center uppercase">
      {name}
    </span>
...
export default function SiteConfigPage() {
  ...
  const { data, mutate, loading } = useAsync<SettingsData>(
    () => '/admin' + SITE_CONFIG,
    true,
  );
  const siteConfig =
    (data?.data as any)?.key === 'site_settings'
    ...
    <>
      ...
      {edit ? (
        ...
        Array.from({ length: 2 }).map((_, idx) => (
          <Skeleton className="my-8" key={idx} />
        ))
      ) : siteConfig ? (
