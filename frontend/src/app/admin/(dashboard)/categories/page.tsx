...
import type { CategoryListItem, TCategories, TCategoryQueryParams } from '../../../../../api-types'
...
const getStatusBadge = (isActive: boolean | null) => {
  ...
  return (
    <Badge className='pt-1 text-xs uppercase' variant={isActive ? 'default' : 'secondary'}>
...
function CategoriesContent() {
  ...
  const { data: categoriesResponse, loading, error, mutate } = useAsync<TCategories>(apiUrl)
  ...
  const actions: ActionItem<CategoryListItem>[] = [
    {
      ...
      href: (item) => `/admin/categories/${item.slug}`
    },
    {
      label: 'Edit',
      icon: <Edit className='w-4 h-4' />,
      href: (item) => `/admin/categories/${item.slug}/edit`
    },
    // {
    //   label: 'View Articles',
  ...
  const handleDelete = async (newsId: number) => {
    try {
      const response = await requests.delete(`/categories/${newsId}`)
      if (response.success) {
        mutate()
